...
resource "aws_ecs_cluster" "main" {
  ...
  setting {
    ...
    value = "disabled" # Mantido disabled em staging para economia
  }
}
...
resource "aws_iam_role" "ecs_execution_role" {
  name = "${var.app_name}-ecs-exec-${var.environment}"
...
resource "aws_security_group" "alb_sg" {
  ...
  description = "Permitir entrada HTTP para o Application Load Balancer"
...
resource "aws_security_group" "ecs_tasks_sg" {
  name        = "${var.app_name}-tasks-sg-${var.environment}"
  description = "Permitir conexoes do ALB para as tasks ECS"
  vpc_id      = data.aws_vpc.default.id

  ingress {
    description     = "Porta da API Fastify pelo ALB"
    from_port       = 3000
    to_port         = 3000
    protocol        = "tcp"
